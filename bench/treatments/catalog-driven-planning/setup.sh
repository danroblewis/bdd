#!/bin/bash
# Setup script for the catalog-driven-planning treatment
# Like full-bdd but with ALL MCP tools enabled (including bdd_add, bdd_tree,
# bdd_next, bdd_motivation). The agent is instructed to use bdd_add to plan
# new features in catalog terms before coding.
...
# catalog-driven-planning needs bdd_add, bdd_tree, bdd_next, bdd_motivation
# all available so the agent can plan in catalog terms before coding
cat > .mcp.json << EOF
{
  "mcpServers": {
    "bdd": {
...
echo "BDD system initialized (catalog-driven-planning): catalog.json, bdd.json, .mcp.json (all tools), hooks, index"
