#!/bin/bash
# Setup script for whw-plus-bdd-test treatment
# Creates catalog + bdd.json + .mcp.json with ONLY bdd_test tool exposed.
# No hooks, no extra MCP tools — just bdd_test on top of why-how-what context.
...
# --- catalog.json (needed by bdd_test to update facet statuses) ---
...
# --- bdd.json (test config — cobertura coverage for bdd_test) ---
cat > bdd.json << EOF
{
...
# --- .mcp.json (registers bdd_server.py with ONLY bdd_test exposed) ---
# Exclude all tools except bdd_test
cat > .mcp.json << EOF
{
  "mcpServers": {
    "bdd": {
      "command": "$VENV_PYTHON",
      "args": ["$BDD_SERVER", "$WORKSPACE", "--exclude-tools", "bdd_next,bdd_motivation,bdd_tree,bdd_status,bdd_locate,bdd_add,bdd_link,bdd_check"]
    }
  }
}
EOF
...
echo "whw-plus-bdd-test initialized: catalog.json, bdd.json, .mcp.json (bdd_test only)"
