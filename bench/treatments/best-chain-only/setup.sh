#!/bin/bash
# Setup script for best-chain-only treatment
# Same as full-bdd but uses best-chain hook variants that inject only
# the single most relevant motivation chain per file access.
...
# --- .claude/settings.json (best-chain hooks) ---
mkdir -p .claude/hooks
cp "$HOOK_SCRIPT" .claude/hooks/inject-context-best-chain.sh
cp "$WRITE_HOOK_SCRIPT" .claude/hooks/inject-write-context-best-chain.sh
chmod +x .claude/hooks/inject-context-best-chain.sh .claude/hooks/inject-write-context-best-chain.sh
...
hooks.setdefault('PostToolUse', []).extend([
    {
        ...
        'hooks': [{
            'type': 'command',
            'command': '$VENV_PYTHON \"\$CLAUDE_PROJECT_DIR/.claude/hooks/inject-context-best-chain.sh\" 2>/dev/null || true'
        }]
    },
    {
        'matcher': 'Edit|Write',
        'hooks': [{
            'type': 'command',
            'command': '$VENV_PYTHON \"\$CLAUDE_PROJECT_DIR/.claude/hooks/inject-write-context-best-chain.sh\" 2>/dev/null || true'
        }]
    }
])
...
echo "best-chain-only initialized: catalog.json, bdd.json, .mcp.json, best-chain hooks, index"
