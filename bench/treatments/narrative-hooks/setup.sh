#!/bin/bash
# Setup script for narrative-hooks treatment
# Same as full-bdd but uses narrative hook variants that render motivation
# as natural-language design notes instead of structured trees.
...
HOOK_SCRIPT="$(cd "$(dirname "$0")/../../.." && pwd)/framework/hooks/inject-context-narrative.sh"
WRITE_HOOK_SCRIPT="$(cd "$(dirname "$0")/../../.." && pwd)/framework/hooks/inject-write-context-narrative.sh"
...
# --- .claude/settings.json (narrative hooks) ---
mkdir -p .claude/hooks
cp "$HOOK_SCRIPT" .claude/hooks/inject-context-narrative.sh
cp "$WRITE_HOOK_SCRIPT" .claude/hooks/inject-write-context-narrative.sh
chmod +x .claude/hooks/inject-context-narrative.sh .claude/hooks/inject-write-context-narrative.sh
...
hooks.setdefault('PostToolUse', []).extend([
    {
        ...
        'hooks': [{
            'type': 'command',
            'command': '$VENV_PYTHON \"\$CLAUDE_PROJECT_DIR/.claude/hooks/inject-context-narrative.sh\" 2>/dev/null || true'
        }]
    },
    {
        'matcher': 'Edit|Write',
        'hooks': [{
            'type': 'command',
            'command': '$VENV_PYTHON \"\$CLAUDE_PROJECT_DIR/.claude/hooks/inject-write-context-narrative.sh\" 2>/dev/null || true'
        }]
    }
])
...
echo "narrative-hooks initialized: catalog.json, bdd.json, .mcp.json, narrative hooks, index"
