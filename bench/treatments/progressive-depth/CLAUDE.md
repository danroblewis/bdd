...
## Context System: Progressive Depth

This project uses a two-level motivation system that matches information depth to what you're doing:

**When you READ a file**, the hook shows a light orientation — just the top-level goal(s) the file serves. This tells you "what area of the project am I in?" without overwhelming detail.

**When you WRITE or EDIT a file**, the hook shows the specific stakeholder expectations at stake — the things users care about that your change must preserve. This is the contract you need to respect.

If you need full implementation details (facet-level), call `bdd_motivation("filename")`.

## Workflow

1. **Read the task prompt** to understand what to implement
2. **Explore source files** — goal-level context orients you to each module's purpose
3. **Implement changes** — expectation-level context shows you what contracts to preserve
4. **Run `bdd_test()`** to execute tests, rebuild the index, and update catalog statuses
5. Call `bdd_motivation()` when you need implementation-level detail about a specific file
