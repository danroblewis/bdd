#!/bin/bash
# Setup script for progressive-depth treatment
# Goal-only hooks on Read, expectation-level hooks on Write/Edit.
# Tests whether matching motivation depth to workflow stage helps.
...
READ_HOOK="$(cd "$(dirname "$0")/../../.." && pwd)/framework/hooks/inject-context-shallow.sh"
WRITE_HOOK="$(cd "$(dirname "$0")/../../.." && pwd)/framework/hooks/inject-context-medium.sh"
...
# --- .claude/settings.json (shallow Read hook + medium Write/Edit hook) ---
mkdir -p .claude/hooks
cp "$READ_HOOK" .claude/hooks/inject-context-shallow.sh
cp "$WRITE_HOOK" .claude/hooks/inject-context-medium.sh
chmod +x .claude/hooks/inject-context-shallow.sh .claude/hooks/inject-context-medium.sh
...
hooks.setdefault('PostToolUse', []).extend([
    {
        ...
        'hooks': [{
            'type': 'command',
            'command': '$VENV_PYTHON \"\$CLAUDE_PROJECT_DIR/.claude/hooks/inject-context-shallow.sh\" 2>/dev/null || true'
        }]
    },
    {
        'matcher': 'Edit|Write',
        'hooks': [{
            'type': 'command',
            'command': '$VENV_PYTHON \"\$CLAUDE_PROJECT_DIR/.claude/hooks/inject-context-medium.sh\" 2>/dev/null || true'
        }]
    }
])
...
echo "progressive-depth initialized: catalog.json, bdd.json, .mcp.json, shallow+medium hooks, index"
